...
export default function ServicesPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {/* =========================================
          MODALS START HERE (100% COMPLETE CODE)
      ============================================= */}
        ...
        <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl w-full max-w-lg border dark:border-gray-700 shadow-2xl">
            ...
            <form onSubmit={handleSubmit((d) => createMutation.mutate(d))} className="space-y-4">
              <input {...register('title', {required: true})} placeholder="Service Title" className="w-full border p-2 rounded dark:bg-gray-900 outline-none" />
              <div className="flex gap-4">
                <select {...register('category', {required: true})} className="w-1/2 border p-2 rounded dark:bg-gray-900 outline-none">
                  <option value="Web Development">Web Development</option>
                  <option value="Graphic Designing">Graphic Designing</option>
                  <option value="Photography">Photography</option>
                  <option value="Tutoring">Tutoring</option>
                ...
                <input {...register('pricing', {required: true})} type="number" placeholder="Price $" className="w-1/2 border p-2 rounded dark:bg-gray-900 outline-none" />
              </div>
              <div className="flex gap-4">
                <input {...register('estimatedDeliveryTime', {required: true})} placeholder="Delivery Time (e.g. 3 Days)" className="w-1/2 border p-2 rounded dark:bg-gray-900 outline-none" />
                <select {...register('availability')} className="w-1/2 border p-2 rounded dark:bg-gray-900 outline-none">
                  <option value="true">Available</option>
                  <option value="false">Busy</option>
                </select>
              </div>
              <textarea {...register('description', {required: true})} rows="3" placeholder="Description" className="w-full border p-2 rounded dark:bg-gray-900 outline-none"></textarea>
              <div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit" disabled={createMutation.isPending} className="w-1/2 bg-green-600 text-white p-2 rounded font-bold hover:bg-green-700 transition">
                  {createMutation.isPending ? 'Saving...' : 'Offer Service'}
                </button>
              </div>
            </form>
          </div>
      ...
      {bookingModal && (
        <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl w-full max-w-md border dark:border-gray-700 shadow-2xl">
            ...
            <p className="text-indigo-600 dark:text-indigo-400 font-bold mb-4 border-b dark:border-gray-700 pb-2">
              {bookingModal.title} - ${bookingModal.pricing}
            </p>
            <form onSubmit={handleBookSubmit} className="space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button type="submit" disabled={bookMutation.isPending} className="w-1/2 bg-indigo-600 text-white p-2 rounded font-bold hover:bg-indigo-700 transition">
              ...
              {sellerReviews.length > 0 ? sellerReviews.map(rev => (
                <div key={rev._id} className="border-b dark:border-gray-800 pb-4 last:border-0 last:pb-0">
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                      <img src={rev.reviewer?.profile?.profilePicture || 'https://via.placeholder.com/40'} className="w-8 h-8 rounded-full" alt="Reviewer" />
                      <div>
